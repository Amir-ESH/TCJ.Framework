...
TCJ provides explicit, bounded resilience primitives for retry, cooperative timeout, isolated circuit breakers, opt-in domain-event handler retries, and transaction-level SQL Server execution-strategy retries. Resilience is never applied blindly: permanent failures and caller cancellation are not retried by default, SQL transaction attempts recreate context/transaction state, and consumer idempotency remains required for repeatable side effects. See [Resilience policies and fault injection](docs/resilience.md).
