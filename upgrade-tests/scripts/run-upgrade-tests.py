...
def run(
    ...
    *,
    ...
    if process.returncode and not allow_failure:
        raise UpgradeError(f"Command failed ({process.returncode}); see {log}")
    return process.returncode, output, warnings
...
def make_env(root: Path) -> tuple[dict[str, str], Path]:
    ...
    if root.exists():
        shutil.rmtree(root)
    for path in (packages, http_cache, home):
...
def parse_assets(path: Path, expected_tcj: list[str], expected_version: str) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    wrong = {key: value for key, value in resolved.items() if value.casefold() != expected_version.casefold()}
    if wrong:
        raise UpgradeError(f"TCJ package version mismatch: {wrong}; expected {expected_version}")
        ...
        for key, item in target_items.items():
            ...
            for group in ("compile", "runtime", "build", "buildTransitive", "analyzers"):
                ...
                if isinstance(values, dict):
                    entry[target_group].update(values.keys())
    for package_id, item in sorted(package_map.items()):
        graph["packages"][package_id] = {
            key: sorted(value) if isinstance(value, set) else value for key, value in item.items()
        }
...
def source_metadata(package_cache: Path, package_id: str, version: str) -> str:
    ...
    if not path.is_file():
        raise UpgradeError(f"Missing NuGet source metadata: {path}")
    source = str(read_json(path).get("source", "")).strip()
    if not source:
        raise UpgradeError(f"Empty NuGet source metadata for {package_id} {version}")
    return source
...
def dependency_diff(baseline: dict[str, Any], target: dict[str, Any]) -> dict[str, Any]:
    baseline_packages, target_packages = baseline["packages"], target["packages"]
    baseline_keys, target_keys = set(baseline_packages), set(target_packages)
    added, removed = sorted(target_keys - baseline_keys), sorted(baseline_keys - target_keys)
    changes: list[dict[str, str]] = []
    upgraded: list[dict[str, str]] = []
    downgraded: list[dict[str, str]] = []
    asset_changes: list[dict[str, Any]] = []
    removed_runtime: list[dict[str, Any]] = []
    for package in sorted(baseline_keys & target_keys):
        baseline_version, target_version = baseline_packages[package]["version"], target_packages[package]["version"]
        if baseline_version != target_version:
            record = {"package": package, "from": baseline_version, "to": target_version}
            changes.append(record)
            try:
                if semver_key(target_version) > semver_key(baseline_version):
                    upgraded.append(record)
                elif semver_key(target_version) < semver_key(baseline_version):
                    downgraded.append(record)
            except UpgradeError:
                pass
        assets: dict[str, Any] = {}
        for group in ("compile", "runtime", "build", "analyzers"):
            before, after = set(baseline_packages[package][group]), set(target_packages[package][group])
            if before != after:
                assets[group] = {"added": sorted(after - before), "removed": sorted(before - after)}
                if group == "runtime" and before - after:
                    removed_runtime.append({"package": package, "assets": sorted(before - after)})
        if assets:
            asset_changes.append({"package": package, "assets": assets})
    return {
        "added": added,
        "removed": removed,
        ...
        "removedRuntimeAssets": removed_runtime,
        "targetFrameworkChanged": baseline["targetFrameworks"] != target["targetFrameworks"],
        "baselineTargetFrameworks": baseline["targetFrameworks"],
        "targetTargetFrameworks": target["targetFrameworks"],
    }
...
def load_behavior(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict) or not isinstance(data.get("checks"), dict):
        raise UpgradeError(f"Invalid behavior JSON: {path}")
    return data
...
def run_phase(
    *,
...
) -> tuple[PhaseResult, dict[str, Any] | None, dict[str, Any] | None]:
    ...
    props = [
        f"-p:TCJUpgradeVersion={version}",
        "-p:TCJUpgradeTargetFramework=net10.0",
        f"-p:RestorePackagesPath={package_cache.resolve()}",
    ...
    try:
        code, _, warnings = run(
            ...
            ROOT,
            env,
            result_dir / "restore.log",
        )
        result.warningCount += warnings
        if code or warnings:
            raise UpgradeError(f"restore exited {code} with {warnings} warning(s)")
        result.restore = "pass"
        assets = project.parent / "obj" / "project.assets.json"
        resolved, graph = parse_assets(assets, scenario["packages"], version)
        assets_copy = result_dir / "project.assets.json"
        shutil.copy2(assets, assets_copy)
        result.assetsFile = str(assets_copy.relative_to(output_root))
        result.packageVersions = resolved
        result.packageSources = verify_sources(
            package_cache,
            scenario["packages"],
            version,
            phase,
        ...
        graph_path = result_dir / "dependency-graph.json"
        graph_path.parent.mkdir(parents=True, exist_ok=True)
        graph_path.write_text(json.dumps(graph, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        result.dependencyGraph = str(graph_path.relative_to(output_root))
        ...
        result.warningCount += warnings
        if code or warnings:
            raise UpgradeError(f"build exited {code} with {warnings} warning(s)")
        result.build = "pass"

        behavior_path = result_dir / "behavior.json"
        run_env = env.copy()
        run_env["TCJ_UPGRADE_BEHAVIOR_PATH"] = str(behavior_path.resolve())
        run_env["TCJ_UPGRADE_PHASE"] = phase
        run_env["TCJ_UPGRADE_DATA_PATH"] = str(
            (output_root / "results" / scenario["name"] / "persisted-data").resolve()
        )
        code, text, warnings = run(
            [
                "dotnet",
                "run",
                "--project",
        ...
        if code or warnings or scenario["expectedOutput"] not in text:
            raise UpgradeError(
                f"runtime exited {code} with {warnings} warning(s); expected {scenario['expectedOutput']!r}"
            )
        behavior = load_behavior(behavior_path)
        expected = load_behavior(ROOT / scenario["expectedBehavior"])
        if behavior != expected:
            raise UpgradeError(f"{phase} behavior differs from expected fixture for {scenario['name']}")
        result.runtime = "pass"
        result.behavior = str(behavior_path.relative_to(output_root))
...
def behavior_changes(baseline: dict[str, Any] | None, target: dict[str, Any] | None) -> list[dict[str, Any]]:
    if baseline is None or target is None:
        return []
    before, after = baseline.get("checks", {}), target.get("checks", {})
    changes: list[dict[str, Any]] = []
    for key in sorted(set(before) | set(after)):
        if before.get(key) != after.get(key):
            changes.append({"check": key, "baseline": before.get(key), "target": after.get(key)})
    return changes


def run_guided_migration(
    scenario: dict[str, Any],
    changes: list[dict[str, Any]],
    target_version: str,
    target_config: Path,
    ...
    if not source_changes:
        return {"required": False, "status": "not-required", "patches": []}
    ...
    for item in source_changes:
        ...
        if not patch:
            return {
                "required": True,
                "status": "fail",
                "failure": f"Missing migration patch for {item.get('id')} and {target_version}",
                "patches": [],
            }
    ...
    try:
        ...
        for patch in patch_paths:
            if not patch.is_file():
                raise UpgradeError(f"Migration patch missing: {patch}")
        ...
        _, change_text, _ = run(
            ...
            source,
            os.environ.copy(),
            migration_root / "source-changes.log",
        )
        files: list[str] = []
        ...
        return {
            "required": True,
            "status": "pass" if phase.restore == phase.build == phase.runtime == "pass" else "fail",
            "patches": [str(path.relative_to(ROOT)) for path in patch_paths],
            "sourceChanges": {"files": files, "insertions": insertions, "deletions": deletions},
            "phase": asdict(phase),
        }
    except UpgradeError as exc:
        return {
...
def select_scenarios(
    ...
    direct = list(policy["scenarios"])
    target_only = list(policy.get("targetOnlyScenarios", []))
    if not requested:
        return direct, target_only
    wanted = set(requested)
    known = {item["name"] for item in [*direct, *target_only]}
    unknown = wanted - known
    if unknown:
        raise UpgradeError(f"Unknown scenario(s): {', '.join(sorted(unknown))}")
    return (
        [item for item in direct if item["name"] in wanted],
        [item for item in target_only if item["name"] in wanted],
    )
...
def main() -> int:
    ...
    if semver_key(args.target_version) <= semver_key(args.baseline_version):
        raise UpgradeError("Target version must be newer than baseline version.")

    direct_scenarios, target_only_scenarios = select_scenarios(policy, args.scenario)
    if args.target_source_mode == "local":
        missing = [
            package
            for package in policy["requiredPackages"]
            if not (args.target_packages / f"{package}.{args.target_version}.nupkg").is_file()
        ]
        if missing:
            raise UpgradeError(f"Target feed is missing package(s): {', '.join(missing)}")
    ...
    (output_root / "report" / "target-only").mkdir(parents=True, exist_ok=True)
    ...
    total_scenarios = len(direct_scenarios) + len(target_only_scenarios)
    suite: dict[str, Any] = {
        "schemaVersion": 1,
        "sourceCommit": args.commit_sha,
        "baselineVersion": args.baseline_version,
        "targetVersion": args.target_version,
        "targetSourceMode": args.target_source_mode,
    ...
    for scenario in direct_scenarios:
        ...
        baseline_phase, baseline_graph, baseline_behavior = run_phase(
            scenario=scenario,
            phase="baseline",
            version=args.baseline_version,
            config=baseline_config,
            env=baseline_env,
            package_cache=baseline_cache,
            output_root=output_root,
            target_packages=args.target_packages.resolve(),
            target_published=False,
        )
        target_phase, target_graph, target_behavior = run_phase(
            scenario=scenario,
            phase="target",
            ...
            output_root=output_root,
            target_packages=args.target_packages.resolve(),
            target_published=args.target_source_mode == "published",
        )
        after_hash = source_tree_hash(scenario_dir)
        ...
        diff_path = output_root / "report" / "dependency-diffs" / f"{scenario['name']}.json"
        ...
        behavior_diff = {
            ...
            "equivalent": baseline_behavior == target_behavior,
            "changes": observed_behavior_changes,
            "manifestChangeIds": [item.get("id") for item in changes],
        }
        behavior_path = output_root / "report" / "behavior-diffs" / f"{scenario['name']}.json"
        behavior_path.parent.mkdir(parents=True, exist_ok=True)
        behavior_path.write_text(json.dumps(behavior_diff, indent=2) + "\n", encoding="utf-8")
        migration = run_guided_migration(
            scenario,
            changes,
            args.target_version,
            target_config,
            ...
            target_cache,
            output_root,
            args.target_packages.resolve(),
            args.target_source_mode == "published",
        )
        ...
        dependency_ok = (
            diff is not None
            and not diff["downgraded"]
            and not diff["removedRuntimeAssets"]
            and not diff["targetFrameworkChanged"]
        )
        source_change_expected = any(item.get("requiresSourceChange") for item in changes)
        if source_change_expected:
            direct_contract_ok = (
                (not target_ok or classification != "Equivalent") and migration["status"] == "pass"
            )
        else:
            direct_contract_ok = (
                target_ok
                and migration["status"] == "not-required"
                and classification
                in {"Equivalent", "Compatible improvement", "Documented change", "Intentional breaking change"}
            )
        ...
        result = {
            ...
            "baseline": asdict(baseline_phase),
            "target": asdict(target_phase),
            "dependencyDiff": diff,
            "behaviorClassification": classification,
            "behaviorChanges": observed_behavior_changes,
            "migration": migration,
            "overall": "pass" if scenario_ok else "fail",
        }
        result_path = output_root / "results" / scenario["name"] / "scenario-result.json"
    ...
    for scenario in target_only_scenarios:
        ...
        source_unchanged = before_hash == after_hash
        target_ok = target_phase.restore == target_phase.build == target_phase.runtime == "pass"
        scenario_ok = source_unchanged and target_ok
        overall &= scenario_ok
        evidence = {
            "schemaVersion": 1,
            "scenario": scenario["name"],
            "kind": "target-only-package-introduction",
            "packages": scenario["packages"],
            "targetVersion": args.target_version,
            "targetSourceMode": args.target_source_mode,
            "packageVersions": target_phase.packageVersions,
            "packageSources": target_phase.packageSources,
            "dependencyGraph": target_graph,
            "behaviorValidated": target_behavior is not None,
            "sourceUnchanged": source_unchanged,
            "overall": "pass" if scenario_ok else "fail",
        }
        (output_root / "report" / "target-only" / f"{scenario['name']}.json").write_text(
        ...
        behavior_path.write_text(
            json.dumps(
                {
                    "scenario": scenario["name"],
                    "classification": "Target-only package introduction",
                    "equivalent": None,
                    "changes": [],
                    "manifestChangeIds": [],
                },
    ...
    (output_root / "results" / "suite-result.json").write_text(
        json.dumps(suite, indent=2) + "\n", encoding="utf-8"
    )
    if overall:
        print(
            f"Upgrade compatibility passed for {total_scenarios} scenario(s): "
            f"{args.baseline_version} -> {args.target_version} ({args.target_source_mode})."
        )
        return 0
    ...
    for result in suite["scenarios"]:
        if result["overall"] == "pass":
            continue
        if result["scenarioKind"] == "target-only":
            print(
                f"  - {result['name']}: target={result['target']['failure']}, "
                f"sourceUnchanged={result['sourceUnchanged']}",
                file=sys.stderr,
            )
        else:
            print(
                f"  - {result['name']}: baseline={result['baseline']['failure']}, "
                f"target={result['target']['failure']}, behavior={result['behaviorClassification']}, "
                f"sourceUnchanged={result['sourceUnchanged']}",
                file=sys.stderr,
            )
    return 1
...
if __name__ == "__main__":
    try:
        ...
        print(f"Upgrade runner failed: {exc}", file=sys.stderr)
        raise SystemExit(1)
