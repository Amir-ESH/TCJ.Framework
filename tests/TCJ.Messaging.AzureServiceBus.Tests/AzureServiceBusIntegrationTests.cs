...
public sealed class AzureServiceBusIntegrationTests
{
    ...
    public async Task DeadLetter_moves_message_to_dead_letter_subqueue_without_payload_in_reason()
    {
        ...
        await using ServiceBusReceiver dlq = env.Client.CreateReceiver(queue, new ServiceBusReceiverOptions { SubQueue = SubQueue.DeadLetter, ReceiveMode = ServiceBusReceiveMode.PeekLock }); ServiceBusReceivedMessage? dead = await dlq.ReceiveMessageAsync(TimeSpan.FromSeconds(5)); Assert.NotNull(dead); Assert.Equal("invalid-contract", dead.DeadLetterReason); Assert.DoesNotContain("sensitive-payload", dead.DeadLetterErrorDescription ?? string.Empty);
    }
    ...
    public async Task Per_message_time_to_live_is_applied()
    {
        ...
        await using ServiceBusReceiver deadLetter = env.Client.CreateReceiver(queue, new ServiceBusReceiverOptions { SubQueue = SubQueue.DeadLetter, ReceiveMode = ServiceBusReceiveMode.PeekLock });
