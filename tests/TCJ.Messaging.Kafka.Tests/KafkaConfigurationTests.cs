...
public sealed class KafkaConfigurationTests
{
    ...
    [Fact] public void Registration_supports_service_provider_validation(){var s=new ServiceCollection();s.AddTcjMessaging();s.AddTcjKafka(o=>o.BootstrapServers="localhost:9092");using var p=s.BuildServiceProvider(new ServiceProviderOptions{ValidateOnBuild=true,ValidateScopes=true});Assert.NotNull(p.GetRequiredService<IMessagePublisher>());}
