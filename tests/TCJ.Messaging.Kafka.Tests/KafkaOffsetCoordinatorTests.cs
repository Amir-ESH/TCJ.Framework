...
public sealed class KafkaOffsetCoordinatorTests
{
    ...
    [Fact] public void Stale_settlement_after_Rebalance_revocation_cannot_commit(){var c=new KafkaOffsetCoordinator();var p=new TopicPartition("t",0);long g=c.Assign(p);c.Register(p,3);c.Revoke(p);Assert.Throws<KafkaStaleSettlementException>(()=>c.Complete(p,3,g));}
}
