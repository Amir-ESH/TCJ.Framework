...
The repository fixture `tests/TCJ.EntityFrameworkCore.NativeAotExperimental` is a project-reference executable. It sets `PublishAot=true` and a concrete `linux-x64` runtime identifier, references `Microsoft.EntityFrameworkCore.Tasks`, enables the EF Core 10 `EFOptimizeContext` integration, opts into `Microsoft.EntityFrameworkCore.GeneratedInterceptors`, and generates the compiled model plus precompiled queries during publish. The fixture configures the SQL Server provider, explicitly registers a generated Guid Strong ID through `StrongIdConversionRegistry`, applies `ApplyStrongIdConversions(...)` and `ApplyTcjSqlServerConventions()`, contains a representative statically analyzable LINQ query, publishes as Native AOT, and starts without opening a database connection. Strong ID registration uses closed generated expressions and model metadata only; it does not scan assemblies or activate generic converter types from runtime `Type` values.
