...
## Azure Service Bus adapter telemetry

`TCJ.Messaging.AzureServiceBus` uses the `TCJ.Messaging.AzureServiceBus` ActivitySource and Meter. Stable activities cover connect, publish/schedule, receive, settlement, lock renewal, session acceptance, and topology validation; metrics cover publish/receive/settlement counts, duration, lock renewal/loss, processor errors, and active sessions. Destination tags are bounded configured entity names. Payloads, credentials, raw application properties, message IDs, and session IDs are not emitted by default. Adapter telemetry tests use in-memory `ActivityListener` and `MeterListener` instances and require no Azure resource.
