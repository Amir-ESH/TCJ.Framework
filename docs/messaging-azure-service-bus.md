...
The emulator fixture predeclares two isolated session-enabled queues, `tcj-s48-session-mapping` and `tcj-s48-session-ordering`. Local emulator CI validates TCJ `OrderingKey` → Service Bus `SessionId` mapping, broker FIFO ordering within one explicitly accepted session, explicit session-lock renewal, and the adapter configuration invariant `MaximumConcurrentCallsPerSession = 1` using `AcceptSessionAsync`. The pinned emulator is not used as acceptance evidence for `ServiceBusSessionProcessor` next-session discovery: repeated CI runs can leave that discovery pending even when explicit acceptance of the same session succeeds. Full `ServiceBusSessionProcessor` discovery/dispatch therefore requires the protected live Azure Service Bus suite. This distinction is intentional; do not silently skip a cloud-only processor check or claim that the emulator validated behavior it did not exercise.
