...
`TCJ.Messaging`, `TCJ.Messaging.RabbitMQ`, `TCJ.Messaging.AzureServiceBus`, and `TCJ.Messaging.Kafka` are new target-only runtime packages in `0.1.0-preview.5`; none has a valid `0.1.0-preview.4` baseline. The `MessagingConsumer` scenario validates the neutral messaging and RabbitMQ package introductions on the target side without establishing a broker connection. The separate `AzureServiceBusConsumer` target-only scenario restores the Azure adapter from the candidate feed and validates explicit registration, descriptor availability, and package dependency closure without requiring cloud credentials. The `KafkaConsumer` target-only scenario does the equivalent package/registration/manual-offset-default validation for `TCJ.Messaging.Kafka` without opening a broker connection. The verifier rejects fabricated baseline results for target-only packages, validates source identity and dependency closure, and reports target-only success independently from direct-upgrade success.
...
Enabling messaging is explicit. Message type names and schema versions are compatibility contracts. Changes to envelope fields, publish outcomes, settlement semantics, failure categories, framework header names, capabilities, or naming rules require compatibility review and migration guidance. Both RabbitMQ and Azure Service Bus adapters must run the applicable Messaging conformance suite before release. Azure Service Bus entity-name changes and session-mode changes additionally require migration guidance because those settings are coupled to broker topology.
